...
void LibInitGlobals() {
    ...
    snort_initializing = true;
    snort_exiting = 0;
    snort_main_thread_pid = 0;
#ifndef WIN32
    snort_main_thread_id = 0;
#endif

#if defined(SNORT_RELOAD) && !defined(WIN32)
    snort_reload = 0;
    snort_swapped = 0;
    snort_reload_thread_created = 0;
    pthread_t snort_reload_thread_id;
    ...
    AlertList = NULL;   /* Alert function list */
    LogList = NULL;     /* Log function list */
    ...
    /* Locals/Private ************************************************************/
    pcap_loop_count = 0;
    pcap_save_queue = NULL;

#if defined(INLINE_FAILOPEN) && !defined(WIN32)
    inline_failopen_thread_running = 0;
    nline_failopen_initialized = 0;
    nline_failopen_pass_pkt_cnt = 0;
#endif
    ...
    reload_signal = false;
#else
    snort_reload_t reload_signal = 0;
    reload_total = 0;
#endif
 ...
 *
 * Purpose:  Entry point when using snort as a library
 *
 */
int SnortLibInit(int argc, char *argv[])
{
#ifndef WIN32
...
int32_t LibProcessPacket(void* user, const DAQ_PktHdr_t* pkthdr, const uint8_t* pkt) {
    return ProcessPacket(&s_packet, pkthdr, pkt, NULL);
};
